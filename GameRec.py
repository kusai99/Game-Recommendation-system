...
def make_recommendation(searchTerms, games = df):
  new_row = games.iloc[-1,:].copy()  
  ...
  return cosine_sim
  # settings.sim_scores = list(enumerate(cosine_sim2[-1,:]))
  # settings.sim_scores = sorted(settings.sim_scores, key=lambda x: x[1], reverse=True)



  # lst = []
  # for i in range(1, 50):
  #   indx = settings.sim_scores[i][0]
    
  #   format_dec = "{:.2f}%".format((((settings.sim_scores[i][1])+1.0)/2.0)*100.0)
...
make_recommendation(searchTerms)
